...
from plaid.model.investments_holdings_get_request import (  # noqa: E402
...
class UnknownInstitutionError(ValueError):
    """Raised when --only names an institution with no matching item."""


def filter_items(items: dict, only: list[str] | None) -> dict:
    """Restrict items to the named institutions.

    An empty or absent selection means "everything" — the normal full sweep.

    Matching is case-insensitive and ignores surrounding whitespace, since the
    names are typed by hand from an error message. An institution with several
    items (two Chase logins, say) returns all of them; dropping one would
    under-fetch and leave a stale balance on the sheet.

    An unmatched name raises rather than returning an empty dict. Fetching
    nothing would exit successfully having written no balances, which is
    indistinguishable from success and worse than a loud failure.
    """
    if not only:
        return items
    ...
    wanted = {norm(name) for name in only}
    selected = {
        item_id: item
        for item_id, item in items.items()
        if norm(item.get("institution_name", "")) in wanted
    }

    matched = {norm(item.get("institution_name", "")) for item in selected.values()}
    unknown = sorted(name for name in only if norm(name) not in matched)
    if unknown:
        available = sorted(
            {
        ...
        raise UnknownInstitutionError(
            f"No linked item for: {', '.join(unknown)}. "
            f"Known institutions: {', '.join(available) or '(none)'}"
        )
...
def get_investment_holdings(access_token: str, institution_name: str) -> tuple:
    ...
    try:
        ...
        response = _get_client().investments_holdings_get(req)
...
def get_plaid_balances(realtime: bool = True, only: list[str] | None = None) -> dict:
    ...
    Args:
        ...
        only: Institution names to restrict the fetch to. None or empty means
            every item. Scoping a real-time retry to the one institution that
            failed costs $0.10 instead of $0.10 x every linked item.
    ...
    for _item_id, item_data in items.items():
        ...
        holdings_sum_by_account: dict[str, float] = {}
        if "investments" in products:
            ...
            for h in holdings:
                ...
                total_assets += value
                holdings_sum_by_account[account_id] = (
                    holdings_sum_by_account.get(account_id, 0.0) + value
                )
        ...
        try:
            ...
            if realtime:
                ...
                response = _get_client().accounts_balance_get(req)
            else:
                req = AccountsGetRequest(access_token=access_token)
                response = _get_client().accounts_get(req)
            ...
            for acc in accounts:
                ...
                # For investment accounts with fetched holdings, the account
                # balance from Plaid (balances.current) reflects total account
                # value (cash + securities). The securities portion is already
                # in all_holdings, so we record only the residual cash to avoid
                # double-counting while preserving any settlement cash balance.
                if account_id in investment_account_ids:
                    held_value = holdings_sum_by_account.get(account_id, 0.0)
                    cash_residual = max(0.0, (current or 0.0) - held_value)
                    account_info = {
                        "institution": institution_name,
                        "name": acc.get("name"),
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  investments/holdings/get is on the monthly Investments\n"
            "  subscription — unlimited calls, no per-call charge.\n\n"
            "Typical usage:\n"
            "  Biweekly spreadsheet update : python plaid_balance.py --force\n"
            "  Dev / debug session         : python plaid_balance.py --cached\n"
            "  Retry one failed institution:\n"
    ...
    parser.add_argument(
        ...
        metavar="INSTITUTION",
        help=(
            "Fetch only this institution (repeatable). Scopes a real-time "
            "retry to the item that failed, so recovering one institution "
            "costs $0.10 instead of $0.10 x every linked item. Only the "
            "fetched rows are written; the rest keep their current values."
        ),
    )
    parser.add_argument(
        "--check",
    ...
    try:
        result = get_plaid_balances(realtime=realtime, only=args.only)
    except UnknownInstitutionError as e:
        print(f"Error: {e}", file=sys.stderr)
        sys.exit(2)

    # Stamp successful real-time fetch. A scoped run covers only some items,
    # so it must not start the rate-limit clock — otherwise a $0.10 retry
    # would block the next full sweep for MIN_FETCH_INTERVAL_HOURS.
    if realtime and not result["errors"] and not args.only:
        record_realtime_fetch()

    fetch_label = "real-time" if realtime else "cached"
    if args.only:
        fetch_label += f", scoped to {', '.join(args.only)}"
    ...
    if args.force:
        ...
        try:
            ...
            print(
                "\n⚠ accounts.yaml not found — balances above will not be written to "
                "the sheet. Create accounts.yaml and paste balances in manually.",
                file=sys.stderr,
            )
            return
        except Exception as exc:
            print(
                f"\n⚠ accounts.yaml could not be loaded ({exc}) — balances above will "
                "not be written. Fix the config and paste manually.",
                file=sys.stderr,
            )
            return
...
def fetch_all_holdings(items: dict) -> list[dict]:
    ...
    for _item_id, item in items.items():
        ...
        try:
            ...
            acct_resp = _get_client().accounts_get(acct_req)
            acct_map = {
