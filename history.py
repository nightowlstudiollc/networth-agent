...
Single source of truth for the balance-history SQLite database. Design notes
live in docs/plans/ in the private repo (not published).
"""
...
def init_schema(db: Database) -> None:
    ...
    Field meanings are documented in docs/plans/ in the private repo.
    """
    db["accounts"].create(
        {
            "id": str,
...
def sync_accounts_from_yaml(
    ...
    *,
    ...
    Args:
        ...
        yaml_path: path to accounts.yaml (ignored when yaml_data is supplied).
        today: ISO date string (e.g. "2026-04-13"). Injected for testability.
        yaml_data: pre-parsed yaml dict. When provided the file is not read,
            avoiding a redundant open when the caller already loaded the file.
    """
    ...
    cross_dupes = plaid_ids & manual_ids
    if cross_dupes:
        raise ValueError(
            "accounts.yaml: IDs appear in both accounts and manual_accounts sections: "
            + str(sorted(cross_dupes))
        )
...
def find_unregistered_ids(db: Database, balances: dict) -> list[str]:
    """Return sheet IDs that have no row in the accounts table.

    Inserting a balance for an unregistered ID violates the balances ->
    accounts foreign key, and sqlite reports only "FOREIGN KEY constraint
    failed" without naming the ID. Callers use this to fail with an
    actionable message before any write is attempted.

    Retired accounts still have a row, so their FK is satisfiable and they
    are not reported — flagging them would push the user to re-add an
    account they deliberately removed from accounts.yaml.
    """
    known = {row["id"] for row in db["accounts"].rows}
    return sorted(aid for aid in balances if aid not in known)
...
def write_snapshot(
    ...
    """
    missing = [i for i, h in enumerate(holdings) if "history_account_id" not in h]
    if missing:
        raise ValueError(
            f"write_snapshot: holdings[{missing[0]}] is missing 'history_account_id'. "
            "Call resolve_holdings_account_ids() before write_snapshot()."
        )
...
def list_snapshots(db: Database, limit: int = 10) -> list[dict]:
    ...
    return list(
        db.query(
            "SELECT id, captured_at, week_of, source, notes"
...
def preceding_snapshot_week(db: Database, before_week: str) -> str | None:
    """Return the week_of of the most recent weekly snapshot strictly before
    before_week, or None if there is no earlier snapshot.

    Used to choose the diff baseline so comparisons follow the actual snapshot
    cadence (e.g. biweekly) instead of assuming a fixed 7-day interval — a
    hardcoded gap silently lands on a week with no snapshot and zeroes every
    "old" balance. week_of is an ISO Monday string, so lexical ordering is
    chronological. The source filter matches weekly_diff so the chosen week is
    one that weekly_diff can actually resolve.
    """
    rows = list(
        db.query(
            "SELECT week_of FROM snapshots"
            " WHERE week_of < ? AND source = 'weekly'"
            " ORDER BY week_of DESC LIMIT 1",
            [before_week],
        )
    )
    return rows[0]["week_of"] if rows else None
...
def weekly_diff(db: Database, week_a: str, week_b: str) -> list[dict]:
    ...
    market and flow are populated for accounts that have Plaid holdings
    data in either week; accounts without holdings data have None for both.
    """
    sql = """
    ...
    out = []
    for r in rows:
        old = r["old_balance"] if r["old_balance"] is not None else 0.0
        new = r["new_balance"] if r["new_balance"] is not None else 0.0
        item = {
            "id": r["id"],
            "label": r["label"],
            "type": r["type"],
