...
"""
    ...
    assert by_id["manual-asset"]["retired_at"] == "2026-04-13"
...
def test_sync_raises_on_cross_section_duplicate_ids(db, tmp_path):
    """An ID in both accounts and manual_accounts is a config error; must raise."""
    import pytest
    from history import sync_accounts_from_yaml

    cross_dup = tmp_path / "cross_dup.yaml"
    cross_dup.write_text(
        """
spreadsheet_id: "t"
accounts:
  - institution: "TestBank"
...
"""
    ...
    with pytest.raises(ValueError, match="shared-id"):
        sync_accounts_from_yaml(db, str(cross_dup), today="2026-04-14")
