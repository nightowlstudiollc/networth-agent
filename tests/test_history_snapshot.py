...
def test_write_snapshot_raises_if_holding_missing_history_account_id(
    db, sample_accounts_yaml
):
    """Holdings without history_account_id produce a clear ValueError before
    the transaction starts, not a cryptic KeyError mid-rollback."""
    from history import write_snapshot, sync_accounts_from_yaml
    ...
    raw_holding = {
        ...
        "price": 500,
        "value": 5000,
        # history_account_id intentionally absent — simulates caller skipping
        # resolve_holdings_account_ids()
    }
    with pytest.raises(ValueError, match="history_account_id"):
        write_snapshot(
            db,
            captured_at="t",
            week_of="2026-04-13",
