...
# PID file for tracking the running server process, so orphaned processes
# from a crashed or killed CLI session can be detected and cleaned up on the
# next launch. Mirrors the pattern in plaid_mcp_proxy.py.
PID_FILE = Path(__file__).parent / ".plaid_link_server.pid"
...
def ensure_plaid_user() -> str | None:
    ...
    try:
        ...
        response = _get_client().user_create(req)
...
# Configure storage file per environment
if PLAID_ENV == "sandbox":
...
def save_items(items):
    ...
    file already restricted, and os.fchmod narrows an existing file that
    another tool may have created at a wider mode — touch(exist_ok=True)
    silently leaves such a file world-readable. Mirrors plaid_status.py.
    """
...
def create_update_link_token():
    ...
    With a non-empty ``additional_products`` list this adds products to the
    item. With an empty/omitted list it is a pure re-authentication (fixes an
    expired login, ITEM_LOGIN_REQUIRED) without touching products.

    The default is re-auth (no products). Defaulting to a product would force
    it onto institutions that don't support it (e.g. investments on Bank of
    America), which Plaid rejects with INVALID_FIELD — the bug this endpoint's
    conditional consent logic exists to avoid. Callers that want to add a
    product must request it explicitly.
    """
    try:
        data = request.get_json(silent=True) or {}
        ...
        mode = (
            f"add products {additional_products}"
            if products_to_add
        ...
        # Only request additional consented products when adding some. A pure
        # re-auth must not set this — forcing an unsupported product (e.g.
        # investments on Bank of America) makes Plaid reject the token with
        # INVALID_FIELD.
        if products_to_add:
...
def exchange_token():
    ...
    try:
        ...
        exchange_response = _get_client().item_public_token_exchange(req)
        ...
        item_response = _get_client().item_get(item_req)
...
def update_item_products():
    ...
    try:
        ...
        # Update consent expiration. The item_get call succeeded, so what it
        # reports is authoritative — including the absence of an expiration.
        # Skipping the None case would leave a stale date in place after a
        # user removes or extends their consent.
        consent_exp = item.get("consent_expiration_time")
        if consent_exp is not None:
            consent_exp = consent_exp.isoformat()
        items[item_id]["consent_expiration"] = consent_exp
...
def write_pid_file() -> None:
    """Write current PID to file so it can be found and stopped later."""
...
def kill_stale_process() -> None:
    """Kill any stale Link server process left over from a previous run.

    Without this, a session that ends via crash or `kill -9` (which no
    handler can intercept) leaves the PID file behind, and the orphaned
    process keeps holding port 8080. The next launch cleans it up instead of
    ...
    try:
        ...
        print(f"Stopped stale Link server process (PID {old_pid})")
        time.sleep(0.5)  # Give it time to exit
    except (ProcessLookupError, ValueError):
        pass  # Process already gone or PID file was invalid
    except PermissionError:
        # PID was recycled and now belongs to a process we don't own
        # (rare on a single-user dev machine, but possible over time).
        # We can't kill it, but we also can't leave a stale PID file
        # pointing at an unrelated process.
        print(f"Warning: could not signal PID {old_pid} (permission denied)")
    finally:
        cleanup_pid_file()


def _handle_shutdown_signal(signum, _frame) -> None:
    """Ensure the PID file is removed on SIGINT/SIGTERM, then exit.

    Flask's development server (Werkzeug) does not install its own
    SIGTERM handler, so without this the process is killed without a
    chance to clean up, leaving a stale PID file (and, if killed
    ungracefully, an orphaned process still bound to the port).
    """
    cleanup_pid_file()
    sys.exit(0)
...
def is_reloader_child() -> bool:
    """True when running inside Werkzeug's debug-reloader child process.

    With FLASK_DEBUG=true, Werkzeug forks a child and re-executes this module
    from __main__ in both processes. Only the parent should own the PID file
    and the shutdown handlers; see install_process_handlers().
    """
    return os.environ.get("WERKZEUG_RUN_MAIN") == "true"


def install_process_handlers() -> None:
    """Claim the PID file and install shutdown handlers — parent process only.

    Skipped in the reloader child for two reasons. The child would otherwise
    overwrite the PID file with its own PID, so kill_stale_process() on the
    next launch would signal the wrong (already-dead) process and leave the
    real server orphaned on port 8080. And the child installing a handler that
    calls sys.exit(0) bypasses Werkzeug's own SIGINT handling, so Ctrl-C skips
    the reloader's graceful shutdown sequence.
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        # The reloader child never wrote the PID file, so it must not remove
        # the parent's copy on exit.
